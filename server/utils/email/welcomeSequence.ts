...
export function renderWelcomeDay0(data: WelcomeTemplateData) {
  const subject = "Welcome to ThreatNoir. Here's what you get."
  const site = normalizeSiteUrl(data.siteUrl)
...
<html><head>${BASE_STYLES}</head><body>
  <div class="wrapper">
    ...
    <div class="card">
      ...
      <p>Glad you're here. ThreatNoir is a curated security intelligence feed for practitioners — built to cut through the noise and give you the signal that matters.</p>
...
<html><head>${BASE_STYLES}</head><body>
  <div class="wrapper">
    ...
    <div class="card">
      ...
      <p>Every ThreatNoir awareness lesson is a root-cause takeaway from a real incident — something you can check for in your own environment before it happens to you.</p>
...
<html><head>${BASE_STYLES}</head><body>
  <div class="wrapper">
    ...
    <div class="card">
      ...
      <p>Email is great, but if you're in a SOC or dev team, ThreatNoir pushes alerts to where you actually work:</p>
      <p>
  ...
  const text = [
    'Go beyond email',
    '',
    'ThreatNoir pushes alerts to Discord, Telegram, custom webhooks, and MCP clients.',
    '',
...
export function renderWeeklyRoundupNotification(data: {
  ...
  const subject = `ThreatNoir Weekly Roundup: ${data.weeklyLabel}`
...
<html><head>${BASE_STYLES}</head><body>
  ...
  const text = [
    `ThreatNoir Weekly Roundup: ${data.weeklyLabel}`,
    '',
