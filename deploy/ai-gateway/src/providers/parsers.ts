import { CATEGORIES } from '../prompts/summarize-article.js'
...
export function parseClassifiedSummaryFromText(params: { title: string; text: string }): ClassifiedSummary {
  ...
  const category = validSlugs.has(String(parsed.category)) ? String(parsed.category) : 'vulnerabilities'
  ...
  const aiSummary = (typeof parsed.summary === 'string' ? parsed.summary.trim() : '') || params.title
